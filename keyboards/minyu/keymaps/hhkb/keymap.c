...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     * |-----------------------------------------------------------|
     * |LShift |  Z|  X|  C|  V|  B|  N|  M|  ,|  .|  /|RShift |Fn | RShift/Fn = Up Right
     * |-----------------------------------------------------------|
     * |    |LAlt|LGui |          Space          |RGui |RAlt |     | RGui/RAlt = Left Down
     * `-----------------------------------------------------------'
     */
    [_BL] = LAYOUT_60_hhkb(
        ...
        KC_LSFT, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, MT(MOD_RSFT, KC_UP), LT(_FN, KC_RGHT),
        KC_LALT, KC_LGUI, KC_SPC, MT(MOD_RGUI, KC_LEFT), MT(MOD_RALT, KC_DOWN)),

    /* HHKB Fn layer
     * ,-----------------------------------------------------------.
     * |Slep|F1| F2| F3| F4| F5| F6| F7| F8| F9|F10|F11|F12|Hlp|Pow|
     * |-----------------------------------------------------------|
     * |Caps |Nxt|Prv|Stp|Ply|Sel|   |   |Psc|Slk|Pus|Up |Ins| Del |      -*- 代表按键透传, 主要准对功能按键
     * |-----------------------------------------------------------|
    ...
    [_FN] = LAYOUT_60_hhkb(
        ...
        KC_TRNS, KC_VOLD, KC_VOLU, KC_MUTE, KC_EJCT, KC_NO, CK_PWD(USER), KC_NO, KC_HOME, KC_PGUP, KC_LEFT, KC_RGHT, KC_TRNS,
        KC_TRNS, TG(_MOUSE), KB_TOGG, NK_TOGG, QK_LOCK, EE_CLR, QK_REP, KC_NO, KC_END, KC_PGDN, KC_DOWN, KC_TRNS, KC_TRNS,
        KC_TRNS, KC_TRNS, KC_NO, KC_WAKE, KC_APP),
    ...
    [_MOUSE] = LAYOUT_60_hhkb(
        ...
        KC_TRNS, KC_TRNS, KC_BTN1, KC_TRNS, KC_APP),
};
